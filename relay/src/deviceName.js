...
// Get a friendly display name from a Bluetooth device name
// "NoHack BoldBadger47" → "BoldBadger47"
// "NOHACK-OAB6" → derive a name from the ID using keyToName
function deviceDisplayName(bluetoothName) {
  if (!bluetoothName) return 'Unknown';
  if (bluetoothName.startsWith('NoHack ')) {
    return bluetoothName.substring(7);
  }
  if (bluetoothName.startsWith('NOHACK-')) {
    // Legacy name — derive a readable name from it
    return keyToName(bluetoothName);
  }
  return bluetoothName;
}

module.exports = { keyToName, deviceDisplayName };
