import {NoHackFile, InnerPayload, TransportCommand, TransportResponse} from '../types';
...
export function createIntroduction(publicKey: string, senderName?: string): NoHackFile {
  const file: NoHackFile = {
    tag: generateTag(),
    nohack: '3',
    id: generateId(),
  ...
  if (senderName) file.senderName = senderName;
  return file;
}
...
export function packInnerPayload(
  ...
  if (senderName) inner.senderName = senderName;
...
// ── Transport protocol serialization (newline-delimited JSON) ───────────────

export function serializeForTransport(obj: TransportCommand | TransportResponse): string {
  return JSON.stringify(obj) + '\n';
}

export function parseTransportLine(line: string): TransportCommand | TransportResponse | null {
  try {
